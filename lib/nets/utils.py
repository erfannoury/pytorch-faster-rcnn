...
class CustomSpatialCrossMapLRN(Module):
    """
    This class has been copied from this repository:
    https://github.com/thnkim/OpenFacePytorch

    This is a simple modification of
    https://github.com/pytorch/pytorch/blob/master/torch/legacy/nn/SpatialCrossMapLRN.py.
    """

    def __init__(self, size, alpha=1e-4, beta=0.75, k=1, gpuDevice=0):
        super(CustomSpatialCrossMapLRN, self).__init__()
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.paddedRatio.resize_(
            channels + self.size - 1, inputHeight, inputWidth)
        ...
        for n in range(batchSize):
            ...
            torch.sum(self.paddedRatio.narrow(
                0, 0, self.size - 1), 0, out=self.accumRatio)
            for c in range(channels):
                self.accumRatio.add_(self.paddedRatio[c + self.size - 1])
                self.gradInput[n][c].addcmul_(-cacheRatioValue,
                                              input[n][c], self.accumRatio)
    ...
    def clearState(self):
        ...
        return super(CustomSpatialCrossMapLRN, self).clearState()
