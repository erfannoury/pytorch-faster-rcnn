...
class Network(nn.Module):
    ...
    def _add_gt_image_summary(self):
        ...
        image = draw_bounding_boxes(
            self._gt_image, self._image_gt_summaries['gt_boxes'],
            self._image_gt_summaries['im_info'])

        return tb.summary.image(
            'GROUND_TRUTH', image[0].astype('float32') / 255.0)
    ...
    def _roi_pool_layer(self, bottom, rois):
        return RoIPoolFunction(
            cfg.POOLING_SIZE, cfg.POOLING_SIZE, 1. / 16.)(bottom, rois)
    ...
    def _anchor_target_layer(self, rpn_cls_score):
        rpn_labels, rpn_bbox_targets, rpn_bbox_inside_weights, \
            rpn_bbox_outside_weights = anchor_target_layer(
    ...
    def _proposal_target_layer(self, rois, roi_scores):
        rois, roi_scores, labels, bbox_targets, bbox_inside_weights, \
            bbox_outside_weights = proposal_target_layer(
    ...
    def _smooth_l1_loss(self, bbox_pred, bbox_targets, bbox_inside_weights,
        ...
        in_loss_box = torch.pow(in_box_diff, 2) * (sigma_2 / 2.) * \
            smoothL1_sign + (abs_in_box_diff - (0.5 / sigma_2)) * \
            (1. - smoothL1_sign)
