...
class SolverWrapper(object):
    ...
    def __init__(self, network, imdb, roidb, valroidb, output_dir, tbdir,
        ...
        self.tbdir = tbdir
        # Simply put '_val' at the end to save the summaries from the
        # validation set
    ...
    def snapshot(self, iter):
        ...
        filename = cfg.TRAIN.SNAPSHOT_PREFIX + \
            '_iter_{:d}'.format(iter) + '.pth'
        filename = os.path.join(self.output_dir, filename)
        torch.save(self.net.state_dict(), filename)
        print('Wrote snapshot to: {:s}'.format(filename))

        # Also store some meta information, random state, etc.
        nfilename = cfg.TRAIN.SNAPSHOT_PREFIX + \
            '_iter_{:d}'.format(iter) + '.pkl'
    ...
    def from_snapshot(self, sfile, nfile):
        ...
        # (TODO xinlei) I have tried my best to find the random states so that
        # it can be recovered exactly. However the Tensorflow state is
    ...
    def find_previous(self):
        sfiles = os.path.join(
            self.output_dir, cfg.TRAIN.SNAPSHOT_PREFIX + '_iter_*.pth')
        sfiles = glob.glob(sfiles)
        sfiles.sort(key=os.path.getmtime)
        # Get the snapshot name in pytorch
        redfiles = []
        for stepsize in cfg.TRAIN.STEPSIZE:
            redfiles.append(os.path.join(
                self.output_dir,
                cfg.TRAIN.SNAPSHOT_PREFIX + '_iter_{:d}.pth'.format(
                    stepsize + 1)))
    ...
    def initialize(self):
        ...
        print('Loaded.')
        # Need to fix the variables before loading, so that the RGB weights
        # are changed to BGR
    ...
    def train_model(self, max_iters):
        ...
        if lsf == 0:
            lr, last_snapshot_iter, stepsizes, np_paths, ss_paths = \
                self.initialize()
        else:
            lr, last_snapshot_iter, stepsizes, np_paths, ss_paths = \
